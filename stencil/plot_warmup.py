...
def parse_manual(path: Path) -> pd.DataFrame:
    ...
    numf = r'(?:[.\d+-]+|--)'
    # accept integers, decimals, scientific notation, and placeholders
    numf1 = r'(?:[-+]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)?|--|nan|NaN|inf|Inf|INF)'
...
def grouped_bar_plot_by_kpct_modes(
    ...
    k_unique = sorted(
        data["Kpct"].dropna().unique().tolist(),
        key=lambda x: (K_ORDER.index(x) if x in K_ORDER else len(K_ORDER), x)
    )
...
def make_plots_for_N_merged_modes(df: pd.DataFrame, N: int, out_dir: Path, threads_value: int):
    ...
    if not d1.empty:
        ...
        # Summary with optional Warmup
        summary_metrics, summary_labels = [], []
        if "Warmup" in d1.columns and d1["Warmup"].notna().any():
            summary_metrics.append("Warmup"); summary_labels.append("Warmup")
        if "End2End" in d1.columns and d1["End2End"].notna().any():
            summary_metrics.append("End2End"); summary_labels.append("End-to-End")
        if "FullE2E" in d1.columns and d1["FullE2E"].notna().any():
            summary_metrics.append("FullE2E"); summary_labels.append("FullE2E (excl. checksum)")
    ...
    if not d2.empty:
        ...
        if any_notna(["Warmup"], d2):
            grouped_bar_plot_by_kpct_modes(
                d2,
                title=f"ALL MODES | Threads={threads_value} | N={N} | Step=2 | Warmup (ms)",
                ylabel="ms",
                outfile=out_dir / f"ALL_N{N}_T{threads_value}_step2_warmup_ms.png",
                metrics=["Warmup"],
                legend_labels=["Warmup"],
            )

        summary_metrics, summary_labels = [], []
        if "Warmup" in d2.columns and d2["Warmup"].notna().any():
            summary_metrics.append("Warmup"); summary_labels.append("Warmup")
        if "End2End" in d2.columns and d2["End2End"].notna().any():
            summary_metrics.append("End2End"); summary_labels.append("End-to-End")
