...
def grouped_bar_plot(
    ...
    metric_hatches = {0: "", 1: "//", 2: "xx", 3: "++"}  # extend if >2 metrics
...
def make_plots_for_N(df: pd.DataFrame, N: int, out_dir: Path, mode_label: str):
    ...
    d1 = dN[dN["Step"] == 1]
    if not d1.empty:
        if d1[["H2D","UMDev","D2H","UMHost"]].notna().any().any():
            grouped_bar_plot(
                d1,
        ...
        if d1[["GPUms","CPUms"]].notna().any().any():
            grouped_bar_plot(
                d1,
        ...
        if d1[["GPUgbps","CPUgbps"]].notna().any().any():
            grouped_bar_plot(
                d1,
        ...
        if d1[["End2End","Read"]].notna().any().any():
            grouped_bar_plot(
                d1,
                title=f"{mode_label} | N={N} | Step=1 | Summary (ms): End-to-End / Read",
                ylabel="ms",
                outfile=out_dir / f"{mode_label}_N{N}_step1_summary_ms.png",
                metrics=["End2End","Read"],
                legend_labels=["End-to-End","Read"],
            )

    # Step 2
    d2 = dN[dN["Step"] == 2]
    if not d2.empty:
        if d2[["H2D","UMDev","D2H","UMHost"]].notna().any().any():
            grouped_bar_plot(
                d2,
        ...
        if d2[["GPUms","CPUms"]].notna().any().any():
            grouped_bar_plot(
                d2,
        ...
        if d2[["GPUgbps","CPUgbps"]].notna().any().any():
            grouped_bar_plot(
                d2,
        ...
        if d2[["End2End","Read"]].notna().any().any():
            grouped_bar_plot(
                d2,
                title=f"{mode_label} | N={N} | Step=2 | Summary (ms): End-to-End / Read",
                ylabel="ms",
