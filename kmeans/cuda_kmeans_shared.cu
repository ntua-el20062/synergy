...
//   int tidx = threadIdx.x;
   // Let only the first thread of the block copy data
  //     if (tidx == 0) {
 ...
 // __syncthreads();
  ...
  if(tid<numObjs){
  int tidx = threadIdx.x; // Thread ID within the block
  int stride = blockDim.x; // Total threads per block

  // Parallel copy: each thread copies a chunk of data
  for (int i = tidx; i < numCoords * numClusters; i += stride) {
      shmemClusters[i] = deviceClusters[i];
   }

   //Synchronize threads to ensure all data is copied before proceeding
  __syncthreads();
  }
...
//    int blockSize = 64;  // default or user-provided block size
 ...
 /*    int minGridSize;
   cudaError_t errors = cudaOccupancyMaxPotentialBlockSize(&minGridSize, &maxThreadsPerBlock, find_nearest_cluster, clusterBlockSharedDataSize, 0);
...
*/
  ...
  fprintf(fp, "%s,%d,%lf,%lf,%lf\n", "Shmem with 2 coords", blockSize, timing / loop, timer_min, timer_max);
  //fprintf(fp, "%s,%d,%lf,%lf,%lf,%lf,%lf,%lf\n", "Shem", blockSize, timing / loop, timer_min, timer_max, 1000 * cpu_time / loop,1000 * gpu_time / loop,1000 * transfers_time / loop);
