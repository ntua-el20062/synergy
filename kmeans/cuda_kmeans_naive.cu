...
__device__ int get_tid() { // __device__ : means its in global memory!!!
  return blockIdx.x * blockDim.x + threadIdx.x ; /* TODO: Calculate 1-Dim global ID of a thread */
}
...
__global__ static
void find_nearest_cluster(int numCoords,            //this is what each thread runs, each thread is assigned a specific object 
                          int numObjs,
                          int numClusters,
                          double *objects,           //  [numObjs][numCoords]
...
/*TODO:Maybe something is missing here,should all threads run this? No, because maybe when initialize the thread blocks,the number of threads created might exceed the number of objects we need to handle*/
  if (tid < numObjs) {
    int index, i;
    ...
    min_dist = euclid_dist_2(numCoords, numObjs, numClusters, objects, deviceClusters, tid, 0); //find the distance from cluster 0
    ...
    if (deviceMembership[tid] != index) {
      ...
      //(*devdelta) += 1.0;
      atomicAdd(devdelta, 1.0); //YES I CAN DO ATOMIC WITH FLOAT
	}
...
//    int blockSize = 64;  // default or user-provided block size
/*
     int minGridSize;
   cudaError_t errors = cudaOccupancyMaxPotentialBlockSize(&minGridSize, &maxThreadsPerBlock, find_nearest_cluster, 0, 0);

    // Print the result and check for errors
    if (errors != cudaSuccess) {
        printf("Error in cudaOccupancyMaxPotentialBlockSize: %s\n", cudaGetErrorString(errors));
        return;
    }

    printf("Max threads per block for find_nearest_cluster: %d\n", maxThreadsPerBlock);
    printf("Minimum grid size required: %d\n", minGridSize);
*/
...
//    printf("tranfer time: %lf\n", transfers_time);
 ...
 printf("Loop: %d transfertime from Device to Host membership: %lf\n",loop, wtime() - timing_transfers);
    /* TODO: Copy dev_delta_ptr to &delta*/
      checkCuda(cudaMemcpy(&delta, dev_delta_ptr, sizeof(double), cudaMemcpyDeviceToHost)); 
    transfers_time += wtime() - timing_transfers;
	//printf("Iteration x transfertime: delta  %lf\n", wtime() - timing_transfers);
  ...
  fprintf(fp, "%s,%d,%lf,%lf,%lf,%lf,%lf,%lf\n", "Naive with 2 coords", blockSize, timing / loop, timer_min, timer_max, 1000 * cpu_time / loop,1000 * gpu_time / loop,1000 * transfers_time / loop);
  fclose(fp);
