...
*/
  ...
  int i;
  for(i = tid; i<numCoords*numClusters; i+=blockDim.x) {
  ...
  if (tid < numObjs) {
    ...
    //edw tha kanoume ton ousiastiko ypologismo twrepei atn new device clusters& new device cluster sizes, kai meta sthn update tha ginei mia aplh antigrafi sthn device clusters
    //devicenewClusterSize[index]++; ston palio kwdika, twra pomic add
    

    atomicAdd(&devicenewClusterSize[index], 1);
    
    //for (j = 0; j < numCoords; j++) devicenewClusters[j][index] += deviceobjects[i*numCoords+j]; ston palio kwdika twra pali prepei me atomic add kai epishs edw to device objects einai[coords][numobj]
    

    int j;   
    for (j = 0; j < numCoords; j++) 
         atomicAdd(&devicenewClusters[j*numClusters+index], deviceobjects[j*numObjs+tid]); //to athroisma se kathe diastasi tou cluster olon ton antikeimenwn gia na bgalw ton meso oro 
    }

  //tha mporousa edw me ena if na bgalw ton meso oro apefthias, alla afou tha ftiaksw neo grid kai blocks klp eidika gia to new clusters, nmzw symferei kalytera na to kanome ekei gt etsi apofeygetai kai to divergence twn threads
}
...
//  fprintf(fp, "%s,%d,%lf,%lf,%lf\n", "All_GPU", blockSize, timing / loop, timer_min, timer_max);
  fprintf(fp, "%s,%d,%lf,%lf,%lf,%lf,%lf,%lf\n", "ALL-GPU", blockSize, timing / loop, timer_min, timer_max, 1000 * cpu_time / loop,1000 * gpu_time / loop,1000 * transfers_time / loop);
