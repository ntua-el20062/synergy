...
void find_nearest_cluster(int numCoords,
	...
	/*TODO:Maybe something is missing here,should all threads run this? No, because maybe when initialize the thread blocks,the number o    f threads created might exceed the number of objects we need to handle*/
	if (tid < numObjs) {
		int index, i;
		...
		min_dist = euclid_dist_2_transpose(numCoords, numObjs, numClusters, objects, deviceClusters, tid, 0); //find the distance from c    luster 0
...
*/
	...
	do {
		...
		checkCuda(cudaMemcpy(deviceClusters, dimClusters[0], numCoords*numClusters*sizeof(double), cudaMemcpyHostToDevice));
		transfers_time += wtime() - timing_transfers;
//		printf("transfers time:%lf\n", transfers_time);
		...
		printf("Loop: %d, cputime %lf\n", loop, wtime() - timing_cpu);
	...
	fprintf(fp, "%s,%d,%lf,%lf,%lf\n", "Transpose with 2 coords", blockSize, timing / loop, timer_min, timer_max);
  //fprintf(fp, "%s,%d,%lf,%lf,%lf,%lf,%lf,%lf\n", "Transpose", blockSize, timing / loop, timer_min, timer_max, 1000 * cpu_time / loop,1000 * gpu_time / loop,1000 * transfers_time / loop);
